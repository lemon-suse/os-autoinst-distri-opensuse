...
sub set_bootscript {
    ...
    my $agama_installer = get_var('AGAMA_INSTALLER', "http://$openqa_url/assets/iso/$agama_iso");
    my $install = is_sle('>=16') ? "root=live:$agama_installer live.password=$testapi::password" : "install=$mirror_http";
...
sub set_bootscript_cmdline_extra {
    ...
    if (is_sle('>=16')) {
        ...
        if (get_var('AGAMA_INSTALL_URL')) {
            ...
            $cmdline_extra .= "agama.install.url=$agama_install_url ";
        }
        if (is_ipmi) {
            my $sol_console = check_var('ARCH', 'aarch64') ? 'ttyAMA0' : 'ttyS1';
